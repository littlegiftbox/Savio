...
echo "wget --no-check-certificate --user=littlegiftbox --password=Give_me_5 "$search_query0 >> $output_file

# Execute the search using wget
wget --no-check-certificate --user=littlegiftbox --password=Give_me_5 "$search_query0" -O ->> $output_file
...
if [ $num_results -eq "100" ]; then
  ...
  wget --no-check-certificate --user=littlegiftbox --password=Give_me_5 "$search_query1" -O ->> $output_file
fi
num_results=`grep 'title>S1' $output_file | wc -l`
if [ $num_results -eq "200" ]; then
  ...
  wget --no-check-certificate --user=littlegiftbox --password=Give_me_5 "$search_query2" -O ->> $output_file
fi
